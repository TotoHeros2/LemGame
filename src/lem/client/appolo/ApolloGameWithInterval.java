...
import lem.client.appolo.ApolloGame.GlobalImageResources;
import lem.client.planete.Lune;
import lem.client.planete.Terre;

public class ApolloGameWithInterval implements EntryPoint, AnimationCallback {
		...
		static final CssColor blueColor = CssColor.make(0, 0, 255);
...
//		private double deltaT = 1d/60d; // calcul reel
		...
		int accelerationTemps = 1;
		InputElement  sliderAccelerationTemps;
		Label accelerationTempsLabel;
...
//			}
		    Timer t = new Timer() {
		        @Override
		        public void run() {
		        	for (int i = 0; i < accelerationTemps;i++)
		        	{
					tempsMission += deltaT;
					ship.nextStep();
					draw();
		      ...
		      // Schedule the timer to run any in n  milliseconds.
...
//			tempsReelLabel = Label.wrap(Document.get().getElementById("tempsReel"));
			...
			// SM + LEM
			double x = lune.distanceTerre*Math.cos(Math.PI/4d)*0.97;// à 97% du chemin
			...
			accelerationTempsLabel = Label.wrap(Document.get().getElementById("accelerationTempsLabel"));
			sliderAccelerationTemps = InputElement.as(Document.get().getElementById("sliderAccelerationTemps"));

			accelerationTempsLabel.setText("1");	
			sliderAccelerationTemps.setValue("1");

			Event.sinkEvents(sliderAccelerationTemps, Event.ONCHANGE);
			Event.setEventListener(sliderAccelerationTemps, new EventListener() {
			    @Override
			    public void onBrowserEvent(Event event) {
					accelerationTempsLabel.setText(sliderAccelerationTemps.getValue());
					accelerationTemps = Integer.valueOf(sliderAccelerationTemps.getValue());;
			    }
			});	
...
//			tempsLabel.setText(String.valueOf(ship.timeMission()/1000.d));
	        ...
	        context.setFillStyle(blueColor);
	        context.setStrokeStyle(blueColor);
